...
class Requests {
    ...
    private static Map getDeepAttributes(def request) {
        ...
        if (request.respondsTo('getRequest') && request != request.getRequest()) { attrs.putAll(getDeepAttributes(request.getRequest())) }
        attrs
    }
    ...
    private static Map getDeepHeaders(def request) {
        ...
        if (request.respondsTo('getRequest') && request != request.getRequest()) { headers.putAll(getDeepHeaders(request.getRequest())) }
        headers
    }
    ...
    private static Map getDeepParameters(def request) {
        ...
        if (request.respondsTo('getRequest') && request != request.getRequest()) {parameters.putAll(getDeepParameters(request.getRequest())) }
        parameters
    }
    ...
    static String getBaseUrl(HttpServletRequest request) {
        String port = request.serverPort == 80 ? '' : ":${request.serverPort}"
